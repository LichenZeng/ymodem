...
#define MAX_RECV_SIZE 48000
#define MAX_SEND_SIZE 48000
...
static int32_t Receive_Packet (uint8_t *data, int32_t *length, uint32_t timeout)
{
	...
	*length = 0;
	...
	{ 
		...
		case SOH:
			printf("Packet_size->128\n");
			packet_size = PACKET_SIZE;
			break;
		case STX:
			printf("Packet_size->1024\n");
		...
		do
		{
			...
			if ((Receive_Byte(&receivedC[0], 100000) == 0)  && (receivedC[0] == ACK))
			{
				...
				if (size > pktSize)
				{
					buf_ptr += pktSize;  
					size -= pktSize;
					if (blkNumber == (MAX_SEND_SIZE/1024))
					{
						return 0xFF; /*  error */
					}
					else
...
int main() {
	uint8_t *buf = (uint8_t *)malloc(24000);
	memset(buf, 0x0, 24000);
	int ret = openPort();
	if(ret) {
		uint32_t i;
		Send_Byte(CRC16);
